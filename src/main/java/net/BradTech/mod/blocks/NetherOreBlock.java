...
public class NetherOreBlock extends Block {
	...
	public boolean removedByPlayer(World world, EntityPlayer player, int x, int y, int z) {
		...
		if(!(hasSilkTouch && BradTech.bradTechConfig.getSilkTouchDisablesPigmen())) {
			NetherOreBlock.angerPigmen(player, world, x, y, z);
		}
	...
	public Item getItemDropped(int metaData, Random random, int fortune) {
		...
		if (this == BradTech.oreNetherBlueQuartzOre) {
			...
			// Return the item of the block if breaking the block DOESN'T drop
			// items.
	...
	public static void angerPigmen(EntityPlayer player, World world, int x, int y, int z) {
		if (BradTech.bradTechConfig.getNetherMiningAngersPigmen()) {
			...
			for (int j = 0; j < list.size(); j++) {
				...
				try {
					Method angryMethod = EntityPigZombie.class.getDeclaredMethod("becomeAngryAt", Entity.class);

					angryMethod.setAccessible(true);
					angryMethod.invoke(list.get(j), (Entity)player);
				} catch (Exception e) {
					System.out.println("Angering the Zombie Pigmen threw an exception. Fucking hell!");
				}
			}
		}
	}
